...
- Generating SBOMs in CycloneDX format with Trivy (or Syft, via `sbom_generator`)
...
| `sbom_generator` | ❌ No | `trivy` | Tool used to auto-generate the SBOM: `trivy` or `syft`. Ignored when `sbom` points to a pre-generated file. |
...
### Auto-generate with Syft instead of Trivy
...
2. **Trivy Java DB Caching**: Fetches and caches the Trivy Java database using multiple repository mirrors to avoid rate limiting (only when `sbom_generator: trivy`)
3. **SBOM Generation**: Scans the image and generates a CycloneDX SBOM unless one is provided. Uses Trivy (v0.70.0) by default, or Syft (v1.51.1 via `anchore/sbom-action`, pinned to CycloneDX 1.6) when `sbom_generator: syft`
...
The attested SBOM is CycloneDX **1.6**: Trivy v0.70.0 emits 1.6 (not configurable), the Syft path is pinned to `cyclonedx-json@1.6` (Syft's own default is 1.7), and the merge step normalizes to `v1_6`.
...
- All dependencies (cosign, Trivy, Syft, ORAS) are pinned to specific versions
