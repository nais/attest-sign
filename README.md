...
| `sbom_lint` | ❌ No | `warn` | How to handle CycloneDX schema validation problems on the final SBOM: `warn` reports them, `error` fails the build, `off` skips validation. |
...
5. **SBOM Validation**: Unless `sbom_lint: off`, runs CycloneDX schema validation on the final SBOM
